...
### 释义：顾名思义；
### 赋值要求：①正整数。如果填写 3 ，表示 recombin_ck_envs1 ~ recombin_ck_envs4 生效；
recombin_ck_envs_num="4"
## 11.2.2 多组环境变量
### 释义：可以自由定义多组局部模式环境变量，如果多组变量中针对同一个脚本设置了多次变量，以第一次变量有效并忽略重复设置的变量；
### 赋值要求：详见“局部模式环境变量”赋值要求；
### recombin_ck_envs1="jd_fruit|jd_pet|jd_plantBean|jd_dreamFactory|jd_jdfactory|jd_crazy_joy|jd_jdzz|jd_jxnc|jd_bookshop|jd_cash|jd_sgmh|jd_cfd|jd_health|jd_carnivalcity|jd_city|jd_moneyTree_heip|zdjr@3@5" ## 强制轮换
### recombin_ck_envs2="jd_jxlhb|jd_88hb@4@5@80@1" # 京喜88元红包组队
